...
def transform_load_data(task_instance):
    data = task_instance.xcom_pull(task_ids="group_a.tsk_extract_Hanoi_weather_data")
    city = data["name"]
    ...
    df_data.to_csv(path_or_buf ="current_weather_data.csv", index=False, header=False)
...
def save_joined_data_s3(task_instance):
    ...
    df = pd.DataFrame(data, columns = ['city', 'description', 'temperature_farenheit', 'feels_like_farenheit', 
                                       'minimun_temp_farenheit', 'maximum_temp_farenheit', 'pressure','humidity',
                                         'wind_speed', 'time_of_record', 'sunrise_local_time', 'sunset_local_time', 
                                         'region', 'area', 'population', 'population_density', 'established'])
    now = datetime.now()
...
def s3_to_postgres():
    s3_hook = S3Hook(aws_conn_id="minio_conn")
    df = pd.read_csv(s3_hook.get_key(
        key=f"vn_city.csv",
        bucket_name="airflow",).get()['Body'])
    print(df)
    df.to_csv(path_or_buf= "vn_city1.csv", index=False, header=False)
    hook = PostgresHook(postgres_conn_id= 'postgres_conn')
    hook.copy_expert(
        sql= "COPY vn_city FROM stdin WITH DELIMITER as ','",
        filename='vn_city1.csv'
    )
...
with DAG('weather_dag_2',
        ...
        with TaskGroup(group_id = 'group_a', tooltip= "Extract_from_S3_and_weatherapi") as group_A:
            create_table_1 = PostgresOperator(
                ...
                postgres_conn_id = "postgres_conn",
                sql= '''  
                    CREATE TABLE IF NOT EXISTS vn_city (
                    city TEXT NOT NULL,
                    Region TEXT NOT NULL,
                    Area numeric NOT NULL,
                    Population numeric NOT NULL,
                    Population_density numeric NOT NULL,
                    Established numeric NOT NULL                       
                );
                '''
            )
            ...
            # )
            uploadS3_to_postgres = PythonOperator(
            ...
            is_Hanoi_weather_api_ready = HttpSensor(
                task_id ='tsk_is_Hanoi_weather_api_ready',
                http_conn_id='weathermap_api',
                endpoint='/data/2.5/weather?q=Hanoi&appid=5798b7354d6fe3eb70592dfdb538e83d'
            )

            extract_Hanoi_weather_data = SimpleHttpOperator(
                task_id = 'tsk_extract_Hanoi_weather_data',
                http_conn_id = 'weathermap_api',
                endpoint='/data/2.5/weather?q=Hanoi&appid=5798b7354d6fe3eb70592dfdb538e83d',
                method = 'GET',
            ...
            transform_load_Hanoi_weather_data = PythonOperator(
                task_id= 'transform_load_Hanoi_weather_data',
            ...
            create_table_2 >> is_Hanoi_weather_api_ready >> extract_Hanoi_weather_data >> transform_load_Hanoi_weather_data >> load_weather_data   
